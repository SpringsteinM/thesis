...
#let template(
  ...
  // let running-header(ctx, element) = {
  ...
  set page(
    ...
    //Eric
    margin: (inside: 2.7cm, outside: 3.5cm, y: 4cm, bottom: 3.5cm),
    header: context {
      if calc.odd(here().page()) {
        align(right, hydra(1))
        // align(right, hydra(custom(<short>), display: running-header))
      } else {
        align(left, hydra(2))
        // align(left, hydra(custom(<short>), display: running-header))
      }
    }
  )
  ...
  show heading.where(level: 2): it => block(above: 2em, below: 1.8em)[
        #counter(heading).display()#h(.5em) #it.body
  ]
  
  show heading.where(level: 3): it => block(above: 2em, below: 1.5em)[
        #counter(heading).display()#h(.5em) #it.body
  ]
  ...
  block[
    ...
    #v(10em)
    #text(1.2em, weight: "bold","Dissertation")
    #v(0.1em)
    #text(1.2em, [
...
der Naturwissenschaften (Dr. rer. nat.) ])
  #v(1em)
    ...
    #text(1.2em, [dem Fachbereich Mathematik und Informatik \
...
vorgelegt von \
  ...
  v(1em, weak: true)
  text[geboren in Rudolstadt]
  v(1em, weak: true)
  text([Marburg, #date.display("[year]")
  ])
